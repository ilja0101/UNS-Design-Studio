...
function updateCards(plants) {
  for (const [key, p] of Object.entries(plants)) {
    ...
    card.className = 'factory-card ' + (running ? 'status-running' : down ? 'status-down' : 'status-stopped');
    maint.textContent = down ? '⚠ Down' : '';
...
async function stopAll() {
  if (!confirm('Stop all plants? (ProcessState=False)')) return;
