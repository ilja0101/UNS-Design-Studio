# factory.py
# Royal Farmers Collective – Enterprise UNS Simulator (STATEFUL ENGINE v3)
#
# Design philosophy:
#   - NO hardcoded tag names. All simulation is purely profile-driven.
#   - Every profile is plant-state-aware (running/fault/recovery/stopped).
#   - The PlantState machine drives all profile behaviour coherently.
#   - Custom tags added via the UNS designer automatically get realistic
#     behaviour based solely on their assigned simulation profile.
#
# Profile catalogue (assign any of these in the UNS designer):
#
#   OT / Process
...
#     pressure             Process pressure (bar / PSI)
#     flow_rate            Flow (m³/h or t/h), zero when stopped
#     level                Tank/silo level (%), drains when running
#     motor_current        Motor current draw (A), spikes on fault
#     vibration            Bearing vibration (mm/s), rises before fault
#     valve_position       Control valve position (%)
#     speed_rpm            Rotational speed, zero when stopped
#     boolean_running      TRUE only when plant is Running
#     boolean_fault        TRUE only when plant is in Fault state
#     boolean_alarm        TRUE during Fault + Recovery
#
#   Accumulators (monotonically increasing when running)
#     accumulator_good     Good product tonnage
#     accumulator_bad      Rejected product tonnage
#     accumulator_energy   Cumulative kWh
#     accumulator_generic  Generic counter (cycles, units, etc.)
#     counter_faults       Fault event counter (increments on each fault)
#
#   Maintenance / CMMS / Reliability
#     mtbf                 Mean time between failures (hours)
#     mttr                 Mean time to repair (minutes)
#     pm_compliance        Preventive maintenance schedule compliance (%)
#     remaining_useful_life Estimated RUL (hours), resets after PM
#     corrective_wo_count  Open corrective work order count
#     maintenance_cost     Cumulative maintenance cost (EUR)
#
#   Quality / Lab
#     quality_metric_pct   First-pass yield / in-spec rate (%)
#     quality_metric_cont  Continuous quality measure (moisture, purity, etc.)
#     quality_hold         Boolean — TRUE when quality out of spec
#     batch_id             String — cycles on each new batch
#     lot_id               String — cycles on each truck/inbound delivery
#
#   Logistics / Supply Chain
#     silo_level           Silo/tank level (%), drains + refills with trucks
#     inbound_tons         Cumulative received tonnage (accumulator)
#     outbound_tons        Cumulative dispatched tonnage (accumulator)
#     truck_id             String — changes on each delivery event
#     days_of_supply       Derived from silo level
#     order_quantity       Integer — varies per order cycle
#     order_status         String — cycles through order lifecycle states
#
#   ERP / Finance
#     erp_order_id         String — cycles on each production order
#     production_cost_eur  Accumulator — energy + labour proxy
#     waste_cost_eur       Accumulator — tied to bad product rate
#     revenue_eur          Accumulator — tied to good product output
#     margin_pct           Derived margin (%), drifts realistically
#
#   Energy / Utilities
#     power_kw             Active power (kW), plant-state-aware base load
#     steam_flow           Steam consumption (kg/h)
#     compressed_air       Compressed air consumption (m³/h)
#     co2_kg               Cumulative CO₂ (accumulator, tied to energy)
#
#   Fallback
#     default              Generic Gaussian walk — always available
...
# ================================================================
# PLANT STATE MACHINE
# One instance per plant_key. Drives all profile-based simulation.
# ================================================================
class PlantState:
    RUNNING  = "Running"
    FAULT    = "Fault"
    ...
    _GROUP_PARAMS = {
        "KnappertjesBV":  {"base_power": 720,  "infeed_rate": 24,  "product_price": 2.80, "unit_cost": 1.60},
        "Vlokkenheim":    {"base_power": 600,  "infeed_rate": 33,  "product_price": 1.20, "unit_cost": 0.65},
        "FritoMaxx":      {"base_power": 820,  "infeed_rate": 28,  "product_price": 0.95, "unit_cost": 0.52},
        "Wortelkracht":   {"base_power": 690,  "infeed_rate": 37,  "product_price": 3.50, "unit_cost": 1.90},
        "DeBietenBende":  {"base_power": 1185, "infeed_rate": 95,  "product_price": 0.55, "unit_cost": 0.35},
    }
    _DEFAULT_PARAMS = {"base_power": 500, "infeed_rate": 20, "product_price": 1.00, "unit_cost": 0.60}
    ...
    def tick(self, externally_running: bool):
        ...
        if self.state == self.RUNNING:
            self.availability        = self._drift(self.availability, 93.0, 0.015, 0.4, 60, 99.5)
            self.performance         = self._drift(self.performance,  94.0, 0.015, 0.5, 55, 100)
            self.quality             = self._drift(self.quality,      97.0, 0.01,  0.3, 70, 100)
            ...
            self.power_kw            = self._drift(self.power_kw, self._base_power, 0.02, self._base_power * 0.015, self._base_power * 0.6, self._base_power * 1.2)
...
# ================================================================
# PROFILE → VALUE  (no tag names, pure profile dispatch)
# ================================================================
def _profile_value(profile: str, ps: PlantState, sim: dict, current_value):
    ...
    # Legacy profile names from v1 (backward compatibility)
    ...
    if p == "oee" or p == "gauss_walk": return round(ps.oee, 2)
...
# ================================================================
# PROFILE CATALOGUE  (read by app.py for the UNS designer dropdown)
# ================================================================
SIMULATION_PROFILES = {
...
# ================================================================
# DYNAMIC OPC-UA ADDRESS SPACE BUILDER  (unchanged logic)
# ================================================================
...
async def run_simulation(variables, anomaly_key_map):
    ...
    while not stop_flag:
        ...
        for pk in plant_keys:
            _get_plant_state(pk, _group_from_key(pk)).tick(sim_state.get(pk, False))
        ...
        for opc_path, (var, sim, plant_key) in list(variables.items()):
            try:
                ...
                ps      = _get_plant_state(plant_key, _group_from_key(plant_key)) if plant_key else PlantState("__global__", "")
...
async def main():
    ...
    print("    OPC UA Server is RUNNING  (stateful profile engine v3)")
